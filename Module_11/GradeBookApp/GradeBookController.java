...
public class GradeBookController {
    ...
    public void saveEntry(TextField firstNameField, TextField lastNameField, TextField courseField, ComboBox<String> gradeComboBox) {
        String first = firstNameField.getText().trim(); // Get first name
        String last = lastNameField.getText().trim(); // Get last name
        String course = courseField.getText().trim(); // Get course name
        String grade = gradeComboBox.getValue(); // Get selected grade

        // Validate input fields
        if (first.isEmpty() || last.isEmpty() || course.isEmpty() || grade == null) {
            showAlert("Error", "Please complete all fields."); // Show error if fields are empty
            return; // Exit method if validation fails
        } // End of validation

        // Create a new Student object
        Student student = new Student(first, last, course, grade);
        // Save the student entry using the service
        try {
            service.saveStudent(student); // Save student entry
            showAlert("Success", "Grade entry saved."); // Show success message
    ...
    private void showAlert(String title, String msg) {
        Alert alert = new Alert(Alert.AlertType.INFORMATION, msg); // Create an alert dialog
